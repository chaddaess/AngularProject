...
export class BodyViewerComponent implements OnInit, OnDestroy {
  ...
  private validMuscleNames = new Set<String>;
  ...
  private initScene() {
    ...
    // You can also limit zoom levels or rotation angles if needed:
  ...
  private loadHumanModel() {
    ...
    loader.load('assets/models/muscles.glb', (gltf: { scene: any; }) => {
      ...
      // Optionally set the scale or position
      humanScene.scale.set(1, 1, 1);
      humanScene.position.set(0, 0, 0);
    ...
    this.routinesService.getMuscles().subscribe({
      next: (muscles: Muscle[]) => {
        muscles.forEach(m => {
          ...
          this.validMuscleNames.add(nameWithUnderscores);
        });
  ...
  private onClick(event: MouseEvent) {
    ...
    if (intersects.length > 0) {
      ...
      console.log('Intersected object:', firstHit);

      // Only proceed if this object’s name is in the set of known muscle names
      if (this.validMuscleNames.has(firstHit.name)) {
        console.log('Clicked muscle is recognized:', firstHit.name);
      } else {
        console.log('Clicked object is not a recognized muscle:', firstHit.name);
      }
    }
  }
